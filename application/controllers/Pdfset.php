...
class Pdfset extends Common_Front_Controller {
	...
	function download(){
      ...
      $pdf->SetAuthor((isset(company_detail()['companyName']) ? company_detail()['companyName']:""));
      $pdf->SetTitle((isset(company_detail()['companyName']) ? company_detail()['companyName']:"").'-'.$job['jobName']);
      $pdf->SetSubject((isset(company_detail()['companyName']) ? company_detail()['companyName']:"").'-'.$job['jobName']);
      $pdf->SetKeywords((isset(company_detail()['companyName']) ? company_detail()['companyName']:"").','.$job['jobName'].',Job Information');
...
/*         $content .= '<tr bgcolor="#EAECF0">';
        ...
        $pdf->lastPage();
        $fileName = (isset(company_detail()['companyName']) ? company_detail()['companyName']:"A")."-".$job['jobName'].strtotime(date("Y-m-d H:i:s")).".pdf";
